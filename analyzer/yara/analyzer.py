...
_LOGGER = get_logger('analyzer.yara', root='helium')
...
async def _yara_process_impl(
    ...
    if config.compiled:
        argv.append('-C')
    argv.extend([str(item) for item in config.rule_files])
    argv.append(str(collection_storage.data_dir))
