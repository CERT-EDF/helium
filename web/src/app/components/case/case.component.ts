...
export class CaseComponent {
  ...
  handleSSEEvent(messageEvent: MessageEvent): void {
    ...
    console.log('rcvd:', event);

    const ext = event.ext;
    switch (event.category) {
      case 'subscribe':
        if (!this.activeUsers.includes(ext.user)) {
          this.activeUsers.push(ext.user);
        }
        break;

      case 'unsubscribe':
        this.activeUsers = this.activeUsers.filter((u) => u !== ext.user);
        break;
      ...
      case 'update_collection': {
        ...
        const index_update_collection = this.displayedCollections.findIndex((c) => c.guid == collection.guid);
        if (index_update_collection > -1) this.displayedCollections.splice(index_update_collection, 1, collection);
        break;
      }
